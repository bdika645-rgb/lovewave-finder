...
const StatsSection = () => {
  ...
  const updateStats = (key: keyof typeof statsContent, value: string) => {
  ...
  const statItems = [
    ...
    },
    {
      ...
      labelKey: "stat5Label" as const,
      label: statsContent.stat5Label,
    },
    {
      icon: Clock,
      value: displayStats.averageMatchTime,
      labelKey: "stat6Label" as const,
      label: statsContent.stat6Label,
    },
  ];

  return (
    <EditableSection sectionName="סטטיסטיקות">
            ...
            {statItems.map((stat, index) => (
              <div key={index} className="text-center">
                ...
                <dt className="text-primary-foreground/70 text-sm mt-1">
                  <InlineEditable
                    value={stat.label}
                    onChange={(v) => updateStats(stat.labelKey, v)}
