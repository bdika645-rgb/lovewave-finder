...
const SuccessStoriesSection = () => {
  ...
  const updateStories = (key: keyof typeof storiesContent, value: string) => {
    updateContent("successStories", { [key]: value });
  };
  ...
  return (
    <EditableSection sectionName="סיפורי הצלחה">
        ...
        <div className="container mx-auto px-6">
          <AnimatedSection className="text-center mb-16">
            <h2 id="success-stories-heading" className="font-display text-4xl md:text-5xl font-bold text-foreground mb-4">
              <InlineEditable
                value={storiesContent.title}
                onChange={(v) => updateStories("title", v)}
                as="span"
              />{" "}
              <InlineEditable
                value={storiesContent.titleHighlight}
                onChange={(v) => updateStories("titleHighlight", v)}
                className="text-gradient"
                as="span"
              />
            </h2>
            <InlineEditable
              value={storiesContent.description}
              onChange={(v) => updateStories("description", v)}
